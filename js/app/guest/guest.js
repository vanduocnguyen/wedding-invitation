...
export const guest = (() => {
    ...
    const showGuestName = () => {
        ...
        const displayGuestName = (name) => {
            ...
            const template = `<small class="mt-0 mb-1 mx-0 p-0">${util.escapeHtml(guestName?.getAttribute('data-message'))}</small><p class="m-0 p-0 font-corinthia text-wedding" style="font-size: 2.25rem">${util.escapeHtml(name)}</p>`;
            util.safeInnerHTML(div, template);

            guestName?.appendChild(div);
        };

        const displayGuestName2 = (name) => {
            const guestName = document.getElementById('guest-name-2');
            const div = document.createElement('div');
            div.classList.add('m-2');
     ...
     * @returns {void}
     */
    const normalizeCorinthiaFont = () => {
        document.querySelectorAll('.font-corinthia').forEach((el) => {
